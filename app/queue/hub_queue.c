...
/*******************************************************************************
  ...
  * 备  注	    ： 空间大小 2M；
*******************************************************************************/
u8 lwip_CopyData2FPGA_BUF2( unsigned short *Data, unsigned short len )
{
    //如果在队列尾，且剩余空间放不下,减去8是因为8个字节放读写地址；
	if( ( ( DSP2FPGA2_ARESTLIMIT - 8 ) <  len ) )
	{
		pDDRShareRam->DSP2FPGA2WritePosition = 0;  //位置放到队列头
		return FALSE;
	}
...
//    xil_printf( "DSP2FPGA2WritePosition = %d\n\r", pDDRShareRam->DSP2FPGA2WritePosition );
...
u8 lwip_readFromFPGAPriBuf(void)
{
	...
	{
        pData->startFlag = DATA_EMPTY;
        if(PriFlag == 1) pDDRShareRam->DSP2FPGA1ReadPosition++;
        else if(PriFlag == 2) pDDRShareRam->DSP2FPGA2ReadPosition++;
        else if(PriFlag == 3) pDDRShareRam->DSP2FPGA3ReadPosition++;
        else if(PriFlag == 4) pDDRShareRam->DSP2FPGA4ReadPosition++;
    	return FALSE;
	}
...
//    writeData2FPGAMPEGReportBUF(pData->type, pData->len, (unsigned short *)pData->Data);
    TxDMA( ( const char * )pData->Data, pData->len + 6 );
...
//    xil_printf("lwip_readFromFPGAPriBuf %d %d %d %d\r\n", pDDRShareRam->DSP2FPGA1ReadPosition,
//    		pDDRShareRam->DSP2FPGA2ReadPosition, pDDRShareRam->DSP2FPGA3ReadPosition,
//			pDDRShareRam->DSP2FPGA4ReadPosition);
